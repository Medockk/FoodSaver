...
//        }
    ...
    fun onEvent(event: RestaurantEvent) {
        when (event) {
            is RestaurantEvent.OnCameraPositionChange -> {
                _state.update { it.copy(cameraPositionModel = event.cameraPosition) }
            }

            is RestaurantEvent.OnEnterpriseMapIconClick -> {
            ...
            is RestaurantEvent.OnPhotoPickerLauncherChange -> {
                _state.update { it.copy(isPickerLauncherOpen = event.value) }
            }

            is RestaurantEvent.OnSelectImage -> {
            ...
            RestaurantEvent.OnCloseEnterpriseSheet -> {
                _state.update {
                    it.copy(
            ...
            RestaurantEvent.OnFindUserClick -> {
                _coords.value?.let {
                    baseChannel.trySend(
                        OnZoom(
                            it.latitude,
