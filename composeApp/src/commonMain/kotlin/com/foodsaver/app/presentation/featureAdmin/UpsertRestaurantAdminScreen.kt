...
import com.foodsaver.app.featureRestaurant.featureEnterprises.presentation.upsertRestaurant.UpsertRestaurantAction
...
import foodsaver.composeapp.generated.resources.restaurant_delete
...
fun UpsertRestaurantAdminScreenRoot(
    ...
    ObserveActions(viewModel.channel) {
        when (it) {
            is UpsertRestaurantAction.OnError -> {}
            UpsertRestaurantAction.OnRestaurantAdded -> { onBackClick() }
        }
    }
}
    ...
    Scaffold(
            ...
            PrimaryTopBar(
                ...
                actions = {
                    if (canDeleteRestaurants) {
                        PrimaryTextButton(
                            onClick = {
                                onEvent(UpsertRestaurantEvent.DeleteRestaurant)
                            }
                        ) {
                            Text(
                                text = stringResource(Res.string.restaurant_delete),
        ...
        LazyColumn(
            ...
            item {
                ...
                UploadImageRow(
                    uris = state.restaurantModel?.photoUris ?: state.photoUris,
                    enabled = canEditRestaurants,
                    onUploadClick = {
